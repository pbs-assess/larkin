...
#'
#' @param a [numeric()] per-capita log population growth rate at low abundance
...
sim <- function(
  ...
  for (i in seq_len(num_iter)) {
    ...
    if (i >= 4) {
      R[i, ] <- p_t[i, ] * S[i] * exp(a + sum(-b * S[i:(i - 3)]) + epsilon[i])
    }
    if (i >= 8) {
...
#     # Update state variables
#     if (i >= 4) {
#       R[i, ] <- p_t[i, ] * S[i] * exp(a + sum(-b * S[i:(i - 3)]) + phi_t[i])
#     }
#     if (i >= 8) {
#       S[i + 1] <- (1 - h) * (R[i - 4, 3] + R[i - 3, 2] + R[i - 2, 1])
#     }
